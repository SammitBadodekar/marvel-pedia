...
searchBox.addEventListener("input", async ()=>{
    ...
    charNames.forEach(element => {
       searchResults.innerHTML += `<p><a href="/character-info?query=${element[1].id}">${element[1].name}</a></p>`
    }); 
      },1000)
})
...
async function randomCaracters(){
    let rawRandonChar = await fetch(`/random-characters?query=${getRandomCharacters()}`);
    let charNameData = await rawRandonChar.json()
    const charNames = Object.entries(charNameData);
    for(i=0;i<30;i++) {
        if(charNames[i][1].thumbnail.path === 'http://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available'|| charNames[i][1].thumbnail.path === 'http://i.annihil.us/u/prod/marvel/i/mg/f/60/4c002e0305708'){
            continue;
        }
...
async function randomEvents(){
    let rawRandomEvents = await fetch(`/random-events?query=${getRandomEvents()}`);
...
randomEvents();
const movies = ["Iron man","The Incredible Hulk","Iron Man 2","Thor","Captain America: The First Avenger","The Avengers","Iron Man 3","Thor: The Dark World","Captain America: The Winter Soldier","Guardians of the Galaxy","Avengers: Age of Ultron","Captain America: Civil War","Doctor Strange","Guardians of the Galaxy Vol. 2","Spider-Man Homecoming","Thor: Ragnarok","Black Panther","Avengers: Infinity War","Ant-Man and the Wasp","Captain Marvel","Avengers: Endgame","Spider-Man: Far from Home","Black Widow","Shang-Chi and the Legend of the Ten Rings","Eternals","Spider-Man: No Way Home","Doctor Strange in the Multiverse of Madness","Thor: Love and Thunder","Black panther Wakanda Forever","Ant-Man and the Wasp: Quantumania"]
const tvShows = ["Wandavision","The Falcon and the Winter Soldier","Loki","What if...?","Hawkeye","Moon Knight",'Ms Marvel',"I Am Groot","The Guardians of the Galaxy: Holiday Special"]
let moviesContainer = document.getElementById("movies")
let tvShowsContainer = document.getElementById("tv-shows")
movies.forEach(element => {
    async function moviesData(){
    let rawCharNameData = await fetch(`https://www.omdbapi.com/?t=${element}&apikey=a664b916`)
    let charNameData = await rawCharNameData.json()
